...
public class BudgetAnalysisInterventionReportQueryHandler : 
    IRequestHandler<BudgetAnalysisInterventionReportQuery, ReportFileVm>
{
    ...
    public BudgetAnalysisInterventionReportQueryHandler(IRegionPersistence regionPersistence,
    ...
    public async Task<ReportFileVm> Handle(BudgetAnalysisInterventionReportQuery request,
        CancellationToken cancellationToken)
    {
        Expression<Func<Domain.Entity.Budgeting.Region, bool>> Where() => x => x.Code == request.Options.Region;

        var budgetAnalysis =
            await _budgetAnalysisService.GetBudgetAnalysisInterventionAsync(request.Options);

        var region = await _regionPersistence.GetAsync(Where());
        ...
        var options = new BudgetAnalysisOptions()
        {
            ...
            ReportTitle = "BUDGET ANALYSIS BY INTERVENTION"
        };
