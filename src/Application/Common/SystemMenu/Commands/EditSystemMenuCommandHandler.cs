...
public class EditSystemMenuCommandHandler : IRequestHandler<EditSystemMenuCommand, SystemMenuCommandResponse>
{
    private readonly ISystemMenuPersistence _systemMenuPersistence;
    private readonly IMapper _mapper;

    public EditSystemMenuCommandHandler(ISystemMenuPersistence systemMenuPersistence, IMapper mapper)
    {
        _systemMenuPersistence = systemMenuPersistence;
        _mapper = mapper;
    }

    public async Task<SystemMenuCommandResponse> Handle(EditSystemMenuCommand request, CancellationToken cancellationToken)
    {
        var response = new SystemMenuCommandResponse();
        var systemMenu = _mapper.Map<Domain.Entity.Common.SystemMenu>(request.SystemMenu);

        var result = await _systemMenuPersistence.EditAsync(systemMenu);
